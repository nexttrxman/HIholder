...
assert.match(migration, /CREATE OR REPLACE FUNCTION complete_first_deposit_from_wallet/);
assert.match(migration, /reference_type = 'usdt_deposit'/);
assert.match(migration, /asset = 'USDT'/);
assert.match(migration, /l\.amount >= 1/);
assert.match(migration, /complete_first_deposit_from_wallet\(TEXT\)/);
